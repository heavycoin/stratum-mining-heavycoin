...
class MiningService(GenericService):
    ...
    def submit(self, worker_name, work_id, extranonce2, ntime, nonce, vote = None):
        '''Try to solve block candidate using given parameters.'''
        session = self.connection_ref().get_session()
        session.setdefault('authorized', {})
       
        if vote:
            print "service::submit() vote = " + vote;
        ...
        try:
            (block_header, block_hash, share_diff, on_submit) = Interfaces.template_registry.submit_share(job_id,
             worker_name, session, extranonce1_bin, extranonce2, ntime, nonce, difficulty, vote)
    ...
    if settings.COINDAEMON_ALGO == 'heavy':
        submit.params.append(('vote', 'string', '16bit integer, hex-encoded, big-endian'))
