...
class MiningSubscription(Subscription):
    ...
    def on_template(cls, is_new_block):
        ...
        if settings.COINDAEMON_ALGO == 'heavy':
            (job_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, _, nreward, nmaxvote) = \
        ...
        for subscription in Pubsub.iterate_subscribers(cls.event):
            try:
        		...
        		if settings.COINDAEMON_ALGO == 'heavy':
                            subscription.emit_single(work_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, clean_jobs, nreward, nmaxvote)
                        else:
                            subscription.emit_single(work_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, clean_jobs)
                    else:
        		if settings.COINDAEMON_ALGO == 'heavy':
                            subscription.emit_single(job_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, clean_jobs, nreward, nmaxvote)
                        else:
    ...
    def _finish_after_subscribe(self, result):
        ...
        clean_jobs = True
        if settings.COINDAEMON_ALGO == 'heavy':
                self.emit_single(job_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, True, nreward, nmaxvote)
	else:
