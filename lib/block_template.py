...
class BlockTemplate(halfnode.CBlock):
    ...
    def build_broadcast_args(self):
        ...
        if settings.COINDAEMON_ALGO == 'heavy':
            nreward = binascii.hexlify(struct.pack(">H", self.nReward))
            nmaxvote = binascii.hexlify(struct.pack(">H", self.nMaxvote))
        clean_jobs = True
        
        if settings.COINDAEMON_ALGO == 'heavy':
            return (job_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, clean_jobs, nreward, nmaxvote)
        else:
    ...
    def finalize(self, merkle_root_int, extranonce1_bin, extranonce2_bin, ntime, nonce, vote = None):
