...
class CTransaction(object):
    ...
    def calc_heavy(self):
        if self.heavy is None:
            self.heavy = uint256_from_str(heavycoin_hash.getHash(self.serialize(), len(self.serialize())))
        return self.heavy
...
class CBlock(object):
    ...
    def deserialize(self, f):
        ...
        if settings.COINDAEMON_ALGO == 'heavy':
            self.nVote = struct.unpack("<H", f.read(2))[0]
            self.nReward = struct.unpack("<H", f.read(2))[0]
    ...
    def is_valid(self):
        ...
        while len(hashes) > 1:
            ...
            for i in xrange(0, len(hashes), 2):
                ...
                if settings.COINDAEMON_ALGO == 'heavy':
                    newhashes.append(heavycoin_hash.getHash(self.serialize(), len(self.serialize())))
                else:
    ...
    def __repr__(self):
        if settings.COINDAEMON_ALGO == 'heavy':
            return "CBlock(nVersion=%i hashPrevBlock=%064x hashMerkleRoot=%064x nTime=%s nBits=%08x nNonce=%08x nVote=%08x nReward=%08x vtx=%s)" % (self.nVersion, self.hashPrevBlock, self.hashMerkleRoot, time.ctime(self.nTime), self.nBits, self.nNonce, self.nVote, self.nReward, repr(self.vtx))
        else:
